...
class BookInventory:
    ...
    def sort_inventory(self):
        if self.count_books() < 2:
            print("To sort the inventory, you must add at least 2 books.")
            return
        ...
        books.sort(key=lambda book: book.titles[0].lower())

        self.head = books[0]
        self.tail = books[-1]

        for i in range(len(books)):
            books[i].next_book = books[i + 1] if i + 1 < len(books) else None
            books[i].prev_book = books[i - 1] if i > 0 else None

        print("The Inventory has been sorted alphabetically by their title.")
            ...
            if choice == '1':
                ...
                if auth.login():
                    while True:
                        ...
                        print("5. Sort book inventory from A-Z")
                        print("6. Logout")
                        choice = input("\nEnter your choice: ")
                ...
                mastercode = "1234"
                code = input("Enter the master code to have admin access: ").strip()
                if code == mastercode:
